...
def from_dict_to_ifc_properties(model: BuildingInformationModel, properties: dict) -> list[Base]:
    ...
    for key, value in properties.items():
        if isinstance(value, dict):
            ...
            ifc_property = model._create("IfcComplexProperty", Name=key, UsageName="{}", HasProperties=subproperties)
            ...
            nominal_value = model._create_value(value)
            ifc_property = model._create("IfcPropertySingleValue", Name=key, NominalValue=nominal_value)
            ifc_properties.append(ifc_property)
        else:
...
def from_dict_to_pset(model: BuildingInformationModel, properties: dict, name: str = None) -> Base:
    ...
    pset = model._create("IfcPropertySet", Name=name, HasProperties=ifc_properties)
    return pset
